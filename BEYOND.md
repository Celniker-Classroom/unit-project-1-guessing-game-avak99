...
- **Description**: Checks if the guess is a valid number between 1 and the selected range. If invalid, shows an error message and clears the input without counting it as a guess.
- **Why it improves the game**: Prevents invalid guesses from being processed, provides clear feedback, and maintains game integrity.
